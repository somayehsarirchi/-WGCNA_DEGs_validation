...
# Set file paths
input_pathway_file <- "../results/Full_Common_Pathways.csv"
input_hubgene_file <- "../results/Top20_Shared_HubGenes.xlsx"
output_path <- "../results/Filtered_Enriched_Pathways.csv"

# Load pathway data
data <- read.csv(input_pathway_file, header = TRUE)
...
# Calculate number of hub genes in each pathway
data <- data %>%
...
# Print number of final pathways selected
cat("Number of filtered pathways:", nrow(pathway_data), "\n")
...
# Save the final filtered pathways
write.csv(pathway_data, output_path, row.names = FALSE)

cat("Filtered pathways have been saved successfully! ✅\n")